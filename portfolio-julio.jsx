...
const projects = [
  ...
  { icon: "🤖", tag: "Inteligência Artificial", name: "IA para Tomada D'Água", desc: "Modelo de IA premiado em 1º lugar (CTG Brasil 2025) para prevenir engolimento de grades — maior risco operacional em usinas hidrelétricas.", metrics: ["1º Lugar", "CTG Brasil 2025"] },
  { icon: "⚙️", tag: "Migração de Sistema", name: "UHE Garibaldi", desc: "Migração completa dos sistemas de controle com nova arquitetura SCADA e integração de novos protocolos de comunicação.", metrics: ["SCADA", "CLP Siemens"] },
  { icon: "⚡", tag: "Comissionamento", name: "UHE Belo Monte — 611 MW", desc: "Regulação de velocidade de unidades geradoras de grande porte — validações, testes operacionais e ajustes de malha de controle.", metrics: ["611 MW", "Hycon 400"] },
];
...
export default function Portfolio() {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Space Grotesk',sans-serif", background: "#020c14", color: "#d0eef8", minHeight: "100vh", overflowX: "hidden", position: "relative" }}>
      ...
      <section id="sobre" style={{ minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",position:"relative",zIndex:2,padding:"100px 6% 60px" }}>
        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",gap:28,maxWidth:740 }}>
          ...
          <p style={{ color:"rgba(148,218,238,0.5)",lineHeight:1.85,maxWidth:520,fontSize:15 }}>
            Das turbinas do Amazonas às salas de controle mais avançadas do Brasil — transformando energia hídrica em inteligência operacional com automação, código e dados.
          </p>
          ...
          <div style={{ display:"flex",flexWrap:"wrap",justifyContent:"center",gap:0,marginTop:8,background:"rgba(2,18,32,0.75)",border:"1px solid rgba(34,211,238,0.07)",borderRadius:20,overflow:"hidden",backdropFilter:"blur(10px)" }}>
            {[["7+","Anos","de exp."],["12+","Usinas","atendidas"],["611","MW","comissionados"],["1°","Lugar","CTG 2025"]].map(([big,mid,small],i) => (
      ...
      {/* ── SKILLS ── */}
      <section id="skills" style={{ padding:"100px 6%",position:"relative",zIndex:2 }}>
        <FadeIn>
          <div style={{ textAlign:"center",marginBottom:68 }}>
        ...
        {/* Terminal SCADA style grid */}
        <div style={{ maxWidth:1000,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:2 }}>
          {[
            {
              id:"SYS-01", title:"Automação Industrial", icon:"⚙️",
              status:"OPERANDO",
              items:[
                ["CLP Siemens","ATIVO"],["CLP Schneider","ATIVO"],["CLP Rockwell","ATIVO"],
                ["SCADA Elipse","ATIVO"],["Spectrum Power","ATIVO"],
                ["Comissionamento","ATIVO"],["ION 8600/8650","ATIVO"],
              ]
            },
            {
              ...
              status:"OPERANDO",
              items:[
                ["Python","ATIVO"],["Modelos de Predição","ATIVO"],
                ["Dashboards Analíticos","ATIVO"],["Data Engineering Cloud","ATIVO"],
                ["IA em Confiabilidade","ATIVO"],["Análise de Falhas","ATIVO"],
              ]
            },
            {
              ...
              status:"OPERANDO",
              items:[
                ["CAPEX","ATIVO"],["Projetos Executivos","ATIVO"],
                ["Planejamento Físico-Financeiro","ATIVO"],
                ["Documentação Técnica","ATIVO"],["Interface Multidisciplinar","ATIVO"],
              ]
            },
          ].map((sys, si) => (
            <div key={si} style={{
              ...
              borderRadius: si===0 ? "16px 0 0 16px" : si===2 ? "0 16px 16px 0" : "0",
              padding:"0",
              overflow:"hidden",
              backdropFilter:"blur(16px)",
            }}>
              {/* Terminal header */}
              <div style={{ background:"rgba(8,145,178,0.08)",borderBottom:"1px solid rgba(34,211,238,0.1)",padding:"14px 20px",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                  <span style={{ fontFamily:"monospace",fontSize:11,color:"rgba(34,211,238,0.45)",letterSpacing:"0.1em" }}>{sys.id}</span>
                  <span style={{ fontSize:16 }}>{sys.icon}</span>
                  <span style={{ fontFamily:"'Space Grotesk'",fontWeight:700,fontSize:13,color:"#e0f7ff" }}>{sys.title}</span>
                </div>
                <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                  <div style={{ width:6,height:6,borderRadius:"50%",background:"#22d3ee",boxShadow:"0 0 8px #22d3ee" }} />
                  <span style={{ fontFamily:"monospace",fontSize:10,color:"#22d3ee",letterSpacing:"0.15em" }}>{sys.status}</span>
                </div>
              </div>
              {/* Rows */}
              <div>
                {sys.items.map(([name, st], i) => (
                  <div key={i} style={{
                    display:"flex",justifyContent:"space-between",alignItems:"center",
                    padding:"11px 20px",
                    borderBottom:`1px solid rgba(34,211,238,${i===sys.items.length-1?0:0.05})`,
                    background: i%2===0 ? "rgba(34,211,238,0.015)" : "transparent",
                  }}>
                    <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                      <span style={{ fontFamily:"monospace",fontSize:10,color:"rgba(34,211,238,0.25)",minWidth:24 }}>
                        {String(i+1).padStart(2,"0")}
                      </span>
                      <span style={{ fontSize:13,color:"#94daee" }}>{name}</span>
                    </div>
                    <span style={{ fontFamily:"monospace",fontSize:10,fontWeight:700,color:"rgba(34,211,238,0.5)",letterSpacing:"0.1em" }}>
                      ● {st}
                    </span>
                  </div>
                ))}
        ...
        {/* Languages — barra horizontal simples */}
        <div style={{ maxWidth:1000,margin:"32px auto 0" }}>
          <div style={{
            background:"rgba(2,14,26,0.9)",border:"1px solid rgba(34,211,238,0.12)",
            borderRadius:16,padding:"28px 32px",backdropFilter:"blur(16px)",
            display:"flex",flexWrap:"wrap",alignItems:"center",gap:"24px 48px"
          }}>
            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
              <span style={{ fontFamily:"monospace",fontSize:11,color:"rgba(34,211,238,0.4)",letterSpacing:"0.1em" }}>SYS-04</span>
              <span style={{ fontSize:16 }}>🌐</span>
              <span style={{ fontFamily:"'Space Grotesk'",fontWeight:700,fontSize:13,color:"#e0f7ff" }}>Idiomas</span>
            </div>
            {[["Português",100,"Nativo"],["Inglês",90,"Avançado"],["Espanhol",60,"Intermediário"],["Francês",30,"Básico"]].map(([lang,pct,level]) => (
              <div key={lang} style={{ flex:1,minWidth:140 }}>
                <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6 }}>
                  <span style={{ fontSize:13,color:"#94daee",fontWeight:500 }}>{lang}</span>
                  <span style={{ fontFamily:"monospace",fontSize:10,color:"rgba(34,211,238,0.4)" }}>{level}</span>
                </div>
                <div style={{ height:3,background:"rgba(34,211,238,0.07)",borderRadius:2 }}>
                  <div style={{ width:`${pct}%`,height:"100%",background:"linear-gradient(90deg,#0891b2,#22d3ee)",borderRadius:2 }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding:"80px 6%",position:"relative",zIndex:2 }}>
        ...
        <div style={{ display:"flex",flexWrap:"wrap",gap:20,justifyContent:"center",maxWidth:960,margin:"0 auto" }}>
          {[
            ["🎓","Engenheiro de Controle e Automação","UNIFEI — Universidade Federal de Itajubá","2018"],
            ["📊","Ciência de Dados","XP Educação","2023–2027 (em andamento)"],
            ["📋","MBA em Gestão de Projetos","USP / Esalq","2021–2023"],
          ].map(([icon,title,school,year],i) => (
            <FadeIn key={i} delay={i*0.1}>
              <div className="card" style={{ padding:"28px",minWidth:270,flex:1,maxWidth:310 }}>
                <div style={{ fontSize:30,marginBottom:14 }}>{icon}</div>
                <h3 style={{ fontFamily:"'Space Grotesk'",fontWeight:700,fontSize:16,color:"#e0f7ff",marginBottom:6 }}>{title}</h3>
                <p style={{ color:"rgba(148,218,238,0.4)",fontSize:13,marginBottom:14 }}>{school}</p>
        ...
        <FadeIn>
          <div style={{ maxWidth:540,margin:"0 auto",position:"relative",zIndex:1 }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:12,alignItems:"center" }}>
              ...
              {/* WhatsApp */}
              <a className="wa-btn" href="https://wa.me/5532988513212" target="_blank" rel="noopener noreferrer">
                {/* WhatsApp SVG logo */}
                ...
                <span style={{ fontWeight:600 }}>+55 32 9 8851-3212 (WhatsApp)</span>
              </a>

              {/* Email */}
              <a className="contact-link" href="mailto:juliocasagrande27@gmail.com">
                <span style={{ fontSize:20 }}>✉️</span>
                <span>juliocasagrande27@gmail.com</span>
              </a>

              {/* LinkedIn */}
              <a className="contact-link" href="https://linkedin.com/in/julio-casagrande" target="_blank" rel="noopener noreferrer">
                <span style={{ fontSize:20 }}>💼</span>
                <span>linkedin.com/in/julio-casagrande</span>
              </a>
            </div>
          </div>
        </FadeIn>
